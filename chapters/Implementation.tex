...
We did play around with solving the ANSV problem for a specific index only when needed, but found that in most cases, this was only a little faster or much slower.

\section{LZ Factorization}

The final step is to calculate the LZ factorization.

At first, we attempted to follow the parallel algorithm of jshun.
...
Like many more recent works, we found that the calculation of the LPF array at every index to be too computationaly expensive and wasteful, even on the GPU.
...
We propose breaking away from the ideal LZ factorization and using PLZ.
...
We would like to explain the different scenarios to you with an anology.
...
The last example in table \ref{table:example3} shows a chunk size of 3 and a division of 5.
